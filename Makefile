...
test: ## 테스트 (경합 탐지 포함). DB 불필요
	...
	# 셸 도구의 비동기 정리 로직(CDP 타임아웃·연결 끊김)은 Go 테스트가 보지
	# 않는다. 브라우저 없이 도는 검사라 여기 둘 수 있다 — `make ui` 에 두면
	# 오프라인에서는 한 번도 돌지 않는다.
	#
	# 글로브가 안 맞으면 셸이 그대로 넘겨 node 가 깨진다. 그게 맞다: 테스트
	# 파일이 사라졌는데 조용히 통과하면 없는 것과 같다.
	#
	# **`--test-timeout` 이 없으면 기본이 무한이다.** 이 스위트가 지키는 것이
	# 「무한 대기」이므로, 그 대상을 깨뜨렸을 때 검사가 실패 대신 멈추면 안 된다.
	# Node 는 이벤트 루프가 비면 취소해 주지만 타이머가 하나라도 살아 있으면
	# 그 방어는 돌지 않는다 — 실제로 타이머 누수 변이가 그렇게 매달렸다.
	node --test --test-timeout=20000 scripts/*.test.mjs
