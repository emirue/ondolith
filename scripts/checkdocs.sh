...
if [ ! -f "$IO" ]; then
	...
	# 「대조할 수 없다」로 남아 있었다. 헬퍼가 헬퍼를 부르는 것도 따라간다(고정점).
	#
	# **본문 최상위(탭 하나)만 본다.** 함수 전체를 훑던 판은 `refundError` 를
	# `{404, 422}` 로 읽었다 — 그 404 는 「주문이 안 보이면」이라는 **다른 조건의
	# 조기 반환**인데 오류 분기의 결과와 뭉뚱그려졌고, 그러면 그 헬퍼를 타는 행이
