...
func TestDirectoriesAreConfigurableAndResolveBesideTheConfig(t *testing.T) {
	cfgPath := "/opt/ondolith/ondolith.json"

	// 비워 두면 설정 파일 옆이다 — 운영자가 설치 전체를 옮겨도 둘이 함께 간다.
	c := &Config{}
	if got := c.Uploads(cfgPath); got != "/opt/ondolith/uploads" {
		t.Errorf("기본 업로드 경로 = %q", got)
	}
	if got := c.Themes(cfgPath); got != "/opt/ondolith/themes" {
		t.Errorf("기본 테마 경로 = %q", got)
	}

	// 상대 경로도 설정 파일 기준이다. 프로세스의 작업 디렉터리 기준이면
	// systemd 로 띄웠을 때와 손으로 띄웠을 때가 달라진다.
	c = &Config{UploadDir: "data/files", ThemeDir: "skins"}
	if got := c.Uploads(cfgPath); got != "/opt/ondolith/data/files" {
		t.Errorf("상대 업로드 경로 = %q", got)
	}
	if got := c.Themes(cfgPath); got != "/opt/ondolith/skins" {
		t.Errorf("상대 테마 경로 = %q", got)
	}

	// 절대 경로는 그대로 쓴다 — 업로드를 별도 볼륨에 두는 것이 흔한 배치다.
	c = &Config{UploadDir: "/mnt/uploads", ThemeDir: "/mnt/themes"}
	if got := c.Uploads(cfgPath); got != "/mnt/uploads" {
		t.Errorf("절대 업로드 경로 = %q", got)
	}
	if got := c.Themes(cfgPath); got != "/mnt/themes" {
