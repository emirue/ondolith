...
var servedOutsideTree = map[string]bool{
	...
	// P-001 설치 마법사 · P-002 설치 유도: **설치 트리**가 받는다. 운영 트리에
	// 이 둘이 있으면 설치가 끝난 사이트에서 다시 설치를 시작할 수 있다는 뜻이고,
	// 그것이 CLAUDE.md 3번(설치 트리와 운영 트리는 별개)이 막는 것이다.
	// 검증은 `internal/install` 과 cmd/ondolith 의 트리 교체 테스트가 한다.
	"P-001": true,
	"P-002": true,

	// P-903 404 · P-904 500: 라우트가 아니라 **응답 경로**다. 주소가 없어서
	// 그리는 화면에 주소를 줄 수는 없다. 렌더는 notFound/serverError 가 하고,
	// 테마 레이아웃으로 그려지는지는 그 핸들러의 테스트가 본다.
	"P-903": true,
	"P-904": true,

	// A-102 관리자 셸: 메뉴와 레이아웃이다. 자기 주소가 없고 A-1xx~A-6xx 가
