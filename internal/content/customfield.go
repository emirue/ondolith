...
//
// **`is_admin` 은 이제 컬럼이 아니지만 목록에 남긴다** (00006 이 지웠다, W2-01).
// 그 이름의 커스텀 항목은 JSONB 에 `is_admin: true` 를 담게 되는데, 권한을
// 뜻하는 값처럼 **보이면서** 아무 판정에도 쓰이지 않는다 — 지운 컬럼이
// 위험했던 이유가 그대로 재현된다. 권한은 역할이 정한다 (D15).
