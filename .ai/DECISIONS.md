...
| 항목 | 필요 시점 | 상태 |
|---|---|---|
| goth 프로바이더 설정 API | Phase 1 | 미확인 (W1-31 선택 작업과 함께) |

**토스페이먼츠 API 스펙은 2026-08-05 에 공식 문서로 대조를 마쳤다** ([D50](../docs/50-commerce.md)
「검증된 연동 사양」). 요청서와 달랐던 것 셋 — `Idempotency-Key` 헤더의 존재, **웹훅에 서명이
없다는 것**, 취소·조회 엔드포인트 누락 — 은 D50 에 반영했다.

`go-react` 샘플은 확인 대상에서 뺐다. 위 표가 연동에 필요한 전부이고, 샘플 저장소의 유지
상태에 의존할 이유가 없다.
