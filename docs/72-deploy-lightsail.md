...
복원한 DB에 붙을 방법이 없다 — 그리고 `secret_key` 가 거기 있어, 파일 없이는 DB 에 봉인된
PG 시크릿·SMTP 비밀번호·소셜 client_secret 을 풀 수 없다([D60](60-security.md) 「저장된
자격증명」). 그 경우 관리자 화면에서 다시 입력하면 된다.
