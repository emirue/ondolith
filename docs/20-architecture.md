...
internal/
  ...
  admin/               관리자 트리 — 셸 메뉴·설정·페이지·사용자·시스템 정보
  commerce/            주문 상태머신·금액·재고·PG 어댑터. 화면과 무관하게 참인 규칙만
```
...
- 인증·RBAC (FR-2xx) — Phase 0 은 `users.is_admin` 불리언 하나뿐이었다. Phase 1 이 RBAC 으로 교체했고 `00020`이 그 컬럼을 지웠다 (W2-01)
- 관리자 트리 (FR-7xx)
- 훅 포인트 (NFR-401)
